...
@celery.task
def exec_async_yardstick_cmd(msg):
    """Background task to run yardstick command."""
    with app.app_context():
        app.logger.info("yardstick command running ...")
        exec_yardstick_cmd()
        app.logger.info("yardstick command finished.")
        # for i in xrange(10):
        #     time.sleep(1)
        #     app.logger.info("yardstick command running ...")
...
def index():
    ...
    if request.form['submit'] == 'Send':
        # send right away
        exec_async_yardstick_cmd.delay(msg)
        flash('Sending email to {0}'.format(email))
    else:
        # send in one minute
        exec_async_yardstick_cmd.apply_async(args=[msg], countdown=60)
